...
try {
  ...
  db.serialize(() => {
    db.run('CREATE TABLE IF NOT EXISTS users (id INTEGER PRIMARY KEY AUTOINCREMENT, name TEXT, password TEXT, address TEXT)', (err) => {
      if (err) {
  ...
  app.post('/register', (req, res) => {
    ...
    db.get('SELECT * FROM users WHERE name = ?', [name], (err, row) => {
      ...
      db.run('INSERT INTO users (name, password, address) VALUES (?, ?, ?)', [name, password, address], function (err) {
        if (err) {
          return res.status(500).json({ error: err.message });
        }
  ...
  app.post('/delivery-map', async (req, res) => {
    try {
      const { origin, green_stores, yellow_stores, red_stores } = req.body;
